            ...
            <b-form class="p-2" @submit.prevent="handleSubmit(EditEmployee)">
                ...
                <validation-provider #default="validationContext" name="Cost Center Name" rules="required">
                    <b-form-group label="Cost Center Name" label-for="Cost Center Name">
                        <b-form-input id="Cost Center Name" v-model="myemp.name" :state="getValidationState(validationContext)" trim />
                    ...
                    <v-select v-model="myemp.branch_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="branches" :reduce="bra => bra.id" label="name" :clearable="true" input-id="branch">
    ...
    methods: {
        ...
        EditEmployee() {
            const mydata = {
                ...
                branch_id: this.$props.myemp.branch_id,
            }
            const mythis = this
            axios
                .post(`/cost-center/update/${this.$props.myemp.id}`,
                    mydata)
                .then(response => {
