...
<div v-if="mybill != null">
    ...
    <b-card title="Settlement Information">
        <b-row>
            <b-col cols="4">
                <b-form-group label="Supplier" label-for="Supplier">
                    <v-select v-model="mybill.mysuppliers" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="suppliers" label="full_name" :clearable="true" input-id="supplier">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!mybill.supplier" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
            </b-col>
                    <b-col cols="4">
                <b-form-group label="Payee" label-for="Payee">
                    <v-select v-model="mybill.payee" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="payees" label="full_name" :clearable="true" input-id="Payee">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!mybill.payee" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
            </b-col>
    ...
    <b-card title="Settlement Attachment">
        <b-row>
    ...
    <b-card title="Actions">
        <b-row class="mt-4 mb-2">
            ...
            <b-col cols="12" md="6" lg="6" class="text-right">
                <b-button size="lg" variant="outline-secondary" class="mr-4" @click="$router.push('/settlements')">
                    Cancel
                </b-button>
...
<script>
import EditItem from '@/components/uploadSettlement/items/edititem.vue'
...
export default {
    ...
    methods: {
        ...
        DeleteItemHard(id) {
            axios
                .post(`/settlements/items/delete/` + id, {
        ...
        DeleteFileHard(id) {
            axios
                .post(`/settlements/files/delete/` + id, {
        ...
        UpdateBill() {
            ...
            for (var i = 0; i < this.mybill.files.length; i++) {
                if (!this.mybill.files[i].settlement_id) {
            ...
            formDataObj.append('supplier_id', this.mybill.supplier_id)
            formDataObj.append('payee_id', this.mybill.payee_id)

            if (this.mybill.items != null) {
                for (var i = 0; i < this.mybill.items.length; i++) {
                    formDataObj.append('items[' + i + '][bill_type_id]', this.mybill.items[i].bill_type_id)
                    formDataObj.append('items[' + i + '][expense_id]', this.mybill.items[i].expense_id)
            ...
            axios
                .post(`/settlements/update/` + this.mybill.id, formDataObj, {
                ...
                .then(response => {
                    ...
                    this.myloader.close()
                    this.$router.push('/settlement/view/' + this.mybill.id)
