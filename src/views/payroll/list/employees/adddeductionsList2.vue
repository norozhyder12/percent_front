...
<div>
    <b-row class="mb-2">
        ...
        <b-col cols="12" md="12" class="d-flex align-items-center justify-content-end">
            <div>
                <b-button size="md" variant="light" v-b-modal.my-modal2b>
                    <span class="text-nowrap">Add deductions</span>
                </b-button>
            </div>
            <b-modal id="my-modal2b" ref="my-modal2b" centered title="Add deductions" hide-footer @ok="addItem">
        ...
        <b-col cols="12" md="12">
            <b-table hover responsive :per-page="pagination.per_page" :current-page="pagination.current_page" :items="deductions" :fields="fields" class="removetop">
                ...
                <template #cell(actions)="data">
                    ...
                    <div v-if="data.item.id" @click="DeleteEmp(data.item.id)">
                        <feather-icon icon="TrashIcon" />

                    </div>
                    
                    <div v-else @click="removeitem(data.item)">
                        <feather-icon icon="TrashIcon" />
...
export default {
    ...
    methods: {
            ...
            this.boxOne = ''
            this.$bvModal
                .msgBoxConfirm('Please confirm that you want to delete this Deduction.', {
        ...
        DeleteEmpHard(id) {
            axios
                .post(`/payroll-employees/deductions/delete/` + id, {

                    'Content-Type': 'multipart/form-data',

                })
                .then(response => {
                    this.$emit('reftechemp');
        ...
        removeitem(myitem) {
         ...
         removeitemID(myitem) {
            var myarray = this.$props.deductions;
            myarray.splice(myarray.findIndex(item => item.id === myitem.id ), 1)
            this.items = myarray;
