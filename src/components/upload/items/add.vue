        ...
        <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
            ...
            <b-form class="p-2" @submit.prevent="handleSubmit(addItem)">
                <b-form-group label="Type Of Bill" label-for="bill_type">
                    <v-select v-model="bill_type" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="bills_types" label="type_name" :clearable="true" input-id="bill_type">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!bill_type" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
                <b-form-group label="Type Of Expense" label-for="expense_id">
                    <v-select v-model="expense_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="expense_types" label="exp_name" :clearable="true" input-id="expense_id">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!expense_id" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
    ...
    methods: {
        ...
        addItem() {
            const mydata = {
                ...
                expense_name: this.expense_id.exp_name,
