...
<div style="height:100%" class="d-flex align-items-center justify-content-start">
    ...
    <b-modal size="lg" id="statement-modal"  :ref="'statement-modal-invoice'+BillID" centered title="Payment Log" hide-footer @show="resetModal" @hidden="resetModal">
...
export default {
    ...
    methods: {
        ...
        hidedailog() {
             this.$refs['statement-modal-invoice'+this.$props.BillID].hide()
        },
        fetchBill() {
              this.$refs['statement-modal-invoice'+this.$props.BillID].show()
            this.openLoading();
            axios.get('/invoices/statement/' + this.$props.BillID)
                .then(response => {
                    this.myloader.close();
