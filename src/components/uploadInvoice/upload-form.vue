...
<div>
    ...
    <b-card title="Select Customer">
        <b-row>
            <b-col md="6" lg="4">
                <b-form-group label="Customer" label-for="Customer">
                    <v-select v-model="myCustomer" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="customers" label="name" :clearable="true" input-id="customers">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!myCustomer" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
            </b-col>
            <b-col md="6" lg="6">
                 <suppliderDailog @refetch-customers="fetch_suppliers"></suppliderDailog>
                
            </b-col>
        </b-row>
    </b-card>
    <b-card title="Add Invoice Items">
        <b-form ref="form" class="mt-1">
            <b-row>
                <b-col>
...
<script>
import suppliderDailog from '@/components/Dialogs/newCustomer.vue'
import vue2Dropzone, {
    mounted
} from 'vue2-dropzone'
      ...
      AddAttachment() {
        ...
        submit() {
            ...
            if (this.myCustomer == null) {
                this.openNotification('top-right', 'danger', "Validation", "Please Select Customer")
                return;
            }
