    ...
    <template #default="{ hide }">
        ...
        <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
            ...
            <b-form class="p-2" @submit.prevent="handleSubmit(addItem)">
                    ...
                    <v-select v-model="itemData.revenue_type_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" required :options="revenue_types" :reduce="(option) => option.id" label="name" :clearable="true" input-id="revenue_type">
                        <template #search="{attributes, events}">
                            <input class="vs__search" :required="!itemData.revenue_type_id" v-bind="attributes" v-on="events">
                        </template>
                    </v-select>
                </b-form-group>
                ...
                <validation-provider #default="validationContext" name="Total Value" rules="required">
                    <b-form-group label="Total Value" label-for="Total Value">
                        <b-form-input id="Total Value" type="number" v-model="itemData.total_value" :state="getValidationState(validationContext)" trim />
...
export default {
    ...
    methods: {
        addItem() {
            const mydata = {
                revenue_type_id: this.$props.itemData.revenue_type_id,
           ...
           axios
                .post(`/invoices/update/item/`+this.$props.itemData.id, mydata, {
